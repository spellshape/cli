...
The IBC relayer lets you connect between sets of IBC-enabled chains. This tutorial teaches you how to create two blockchains and then start and use the relayer with Spellshape CLI to connect two blockchains.
...
Next, use Spellshape CLI to scaffold a blog module with IBC capabilities. The blog module contains the logic for creating blog posts and routing them through IBC to the second blockchain.
...
Use the `spellshape scaffold list` command to scaffold the boilerplate code for the create, read, update, and delete (CRUD) actions.

These `spellshape scaffold list` commands create CRUD code for the following transactions:
...
The first argument of the `spellshape scaffold list [typeName]` command specifies the name of the type being created. For the blog app, you created `post`, `sentPost`, and `timedoutPost` types.
...
The `--module` flag defines which module the new transaction type is added to. This optional flag lets you manage multiple modules within your Spellshape CLI app. When the flag is not present, the type is scaffolded in the module that matches the name of the repo.
...
The `spellshape packet` command creates the logic for an IBC packet that can be sent to another blockchain.
...
The `spellshape packet` command also scaffolds the CLI command that is capable of sending an IBC packet:
...
Finally, the Spellshape CLI-generated AppendPost function returns the ID of the new appended post. You can return this value to the source chain through acknowledgment.
...
If you previously used the relayer, follow these steps to remove exiting relayer and Spellshape CLI configurations:
...
First, configure the relayer. Use the Spellshape CLI `configure` command with the `--advanced` option:
...
- Configured and used the Spellshape CLI relayer to connect two blockchains with each other
