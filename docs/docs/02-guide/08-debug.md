...
Spellshape chain debug command can help you find issues during development. It uses
...
built by default by the `spellshape chain serve` command or can optionally be
created using the `--debug` flag when running `spellshape chain init` or `spellshape
...
In this short example we will be using Spellshape CLI to create a new blockchain and
