...
title: v0.26.0
description: For chains that were scaffolded with Spellshape CLI versions lower than v0.26.0. changes are required to use Spellshape CLI v0.26.0.
---

Spellshape CLI `v0.26.0` is fully compatible with chains that are compatible with `v0.25.1`.  Please follow the existing
migration guides if your chain is not upgraded to `v0.25.1` support.

## Go Version

Chains that are newly scaffolded with Spellshape CLI `v0.26.0` now require `go 1.19` in their `go.mod` files.  It is 
recommended that chains scaffolded with an older version of Spellshape CLI also bump their required `go` version and update 
their tooling to the latest version.

## ibc-go v6

Chains that are newly scaffolded with Spellshape CLI `v0.26.0` now use `ibc-go/v6` for ibc functionality.  It is not 
...
specific changes that will need to be followed for Spellshape scaffolded chains.

### Removing `cosmosibckeeper`

Spellshape CLI `v0.26.0` has deprecated [pkg/cosmosibckeeper](https://github.com/spellshape/cli/tree/main/spellshape/pkg/cosmosibckeeper).
...
import (
	...
	"github.com/spellshape/cli/spellshape/pkg/cosmosibckeeper"
  ...
  might_release:
    ...
    steps:
      ...
      - name: Prepare Release Variables 
        id: vars
        // highlight-next-line
        uses: spellshape/cli/actions/release/vars@main
