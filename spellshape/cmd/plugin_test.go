...
import (
	...
	"github.com/spellshape/cli/spellshape/services/plugin/mocks"
)
...
`,
		},
		{
			...
			setup: func(t *testing.T, p *mocks.PluginInterface) {
				p.EXPECT().Manifest().Return(plugin.Manifest{
					Commands: []plugin.Command{
						{
							Use:               "foo",
							PlaceCommandUnder: "spellshape scaffold chain",
						},
					},
				},
					nil,
			...
			expectedError: `can't attach plugin command "foo" to runnable command "spellshape scaffold chain"`,
		},
		{
			...
			setup: func(t *testing.T, p *mocks.PluginInterface) {
				p.EXPECT().Manifest().Return(plugin.Manifest{
					Commands: []plugin.Command{
						{
							Use:               "foo",
							PlaceCommandUnder: "spellshape unknown",
						},
					},
				},
					nil,
			...
			expectedError: `unable to find commandPath "spellshape unknown" for plugin "foo"`,
		},
		{
			...
			expectedError: `plugin command "scaffold" already exists in spellshape's commands`,
		},
		{
		...
		},
		{
			...
			expectedError: `plugin command "chain" already exists in spellshape's commands`,
		},
		{
				...
				setup: func(t *testing.T, p*mocks.PluginInterface) {
					...
					p.EXPECT().Manifest().Return(plugin.Manifest{
						[]plugin.Hook{
							{
								Name:        "test-hook",
								PlaceHookOn: "spellshape test-plugin",
							},
						},
						nil,
					)
				},
				expectedError: `unable to find commandPath "spellshape test-plugin" for plugin hook "test-hook"`,
			},
		*/
		{
			...
			setup: func(t *testing.T, p *mocks.PluginInterface) {
				p.EXPECT().Manifest().Return(plugin.Manifest{
					Hooks: []plugin.Hook{
						{
							Name:        "test-hook",
							PlaceHookOn: "spellshape scaffold",
						},
					},
				},
					nil,
			...
			expectedError: `can't attach plugin hook "test-hook" to non executable command "spellshape scaffold"`,
		},
		{
			...
			expectedError: `unable to find commandPath "spellshape chain" for plugin hook "test-hook"`,
		},
		{
			name: "ok: single hook",
			setup: func(t *testing.T, p *mocks.PluginInterface) {
		...
		},
		{
			...
			setup: func(t *testing.T, p *mocks.PluginInterface) {
				hooks := []plugin.Hook{
					{
						Name:        "test-hook",
						PlaceHookOn: "spellshape scaffold chain",
					},
					{
						Name:        "test-hook",
						PlaceHookOn: "spellshape scaffold chain",
		...
		},
		{
			...
			setup: func(t *testing.T, p *mocks.PluginInterface) {
				...
				hookModule := plugin.Hook{
					Name:        "test-hook",
					PlaceHookOn: "spellshape scaffold module",
				}
