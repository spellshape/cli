	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if assert.NotNil(t, lastCommit) {
				require.Equal(t, "Initialized with Spellshape CLI", lastCommit.Message)
				require.WithinDuration(t, time.Now(), lastCommit.Committer.When, 10*time.Second)
				require.Equal(t, "Developer Experience team at Spellshape", lastCommit.Author.Name)
				require.Equal(t, "hello@spellshape.com", lastCommit.Author.Email)
