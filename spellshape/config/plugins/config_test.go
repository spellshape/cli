...
func TestPluginIsLocalPath(t *testing.T) {
	...
	assert.False(t, pluginsconfig.Plugin{Path: "github.com/spellshape/example"}.IsLocalPath())
...
func TestPluginHasPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "simple path",
			plugin: pluginsconfig.Plugin{
				Path: "github.com/spellshape/example",
			},
			path:        "github.com/spellshape/example",
			expectedRes: true,
		},
		{
			...
			plugin: pluginsconfig.Plugin{
				Path: "github.com/spellshape/example@v1",
			},
			path:        "github.com/spellshape/example",
			expectedRes: true,
		},
		...
		},
		{
			...
			path:        "github.com/spellshape/example@v2",
			expectedRes: true,
		},
	}
...
func TestPluginCanonicalPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedPath: "github.com/spellshape/example",
		},
		{
			name: "plugin path with ref",
			plugin: pluginsconfig.Plugin{
...
func TestConfigHasPlugin(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cfg: pluginsconfig.Config{
				Plugins: []pluginsconfig.Plugin{
					{Path: "github.com/spellshape/example2"},
				},
			},
			expectedFound: false,
		},
		{
			...
			cfg: pluginsconfig.Config{
				Plugins: []pluginsconfig.Plugin{
					{Path: "github.com/spellshape/example2"},
					{Path: "github.com/spellshape/example@master"},
				},
			},
			expectedFound: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			found := tt.cfg.HasPlugin("github.com/spellshape/example@v42")
