...
import (
	...
	"github.com/spellshape/cli/spellshape/pkg/protoanalysis"
...
// checkComponentCreated checks if the component has been already created with Spellshape in the project.
...
func checkForbiddenComponentName(name multiformatname.Name) error {
	...
	case
		...
		"tx",
		datatype.TypeCustom:
		return fmt.Errorf("%s is used by Spellshape scaffolder", name.LowerCamel)
	}
...
func checkForbiddenOracleFieldName(name string) error {
	...
	case
		...
		"PREPAREGAS",
		"EXECUTEGAS":
		return fmt.Errorf("%s is used by Spellshape scaffolder", name)
	}
	return nil
}
