...
import (
	...
	"github.com/spellshape/cli/spellshape/pkg/cliui/view/accountview"
...
type (
	...
	// InitAccounts initializes chain accounts from the Spellshape config.
	// InitConfiguration initializes node configuration from the Spellshape config.
	// InitGenesis initializes genesis state for the chain from Spellshape config.
	InitArgs struct {
		InitAccounts      bool
...
func (c *Chain) InitChain(ctx context.Context, initConfiguration, initGenesis bool) error {
	...
	// ovewrite app config files with the values defined in Spellshape's config file
