// Package plugin implements spellshape plugin management.
// An spellshape plugin is a binary which communicates with the spellshape binary
...
import (
	...
	cliexec "github.com/spellshape/cli/spellshape/pkg/cmdrunner/exec"
...
// Remote plugins require to be fetched first, in $HOME/.spellshape/plugins
...
func newPlugin(pluginsDir string, cp pluginsconfig.Plugin, options ...Option) *Plugin {
	...
	// "github.com/spellshape/plugins/plugin1" where "plugin1" is the subpath.
...
func (p *Plugin) clean() error {
	...
	// Clean the cloneDir, next time the spellshape command will be invoked, the
